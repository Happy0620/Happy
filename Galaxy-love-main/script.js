...
// --- MODIFICATION START: IMAGE SETTINGS ---
// Change this number to match how many photos you have in the images folder
const photoCount = 10; 

const defaultHeartImages = Array.from({ length: photoCount }, (_, i) => `images/img${i + 1}.jpg`);
// --- MODIFICATION END ---
...
for (let group = 0; group < numGroups; group++) {
  ...
  img.onerror = () => {
      console.warn(`Could not load image: ${heartImages[group]}`);
  };
...
function onCanvasClick(event) {
  ...
  if (intersects.length > 0) {
    ...
    // --- UPDATED AUDIO LOGIC ---
    // Trigger the audio player we added to index.html
    const audio = document.getElementById('bgMusic');
    const musicBtn = document.getElementById('musicButton');
    
    if (audio) {
        audio.play().then(() => {
            if (musicBtn) musicBtn.innerText = "slap here ⏸️";
        }).catch(err => {
            console.log("Audio autoplay blocked, waiting for manual click");
        });
    }
    // ---------------------------
